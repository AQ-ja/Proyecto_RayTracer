...
def reflectVector(normal, dirVector):
    # R = 2 * ( N . L) * N - L
    reflect = 2 * Alib.punto(normal, dirVector)
    reflect = Alib.multi(reflect, normal)
    reflect = Alib.sub(reflect, dirVector)
    reflect = reflect / Alib.norm(reflect)
    return reflect


def refractVector(normal, dirVector, ior):
    # Snell's Law
    cosi = max(-1, min(1 , Alib.punto(dirVector, normal)))
    etai = 1
    etat = ior
...
class Raytracer(object):
    ...
    def glRender(self):
        ...
        for y in range(0, self.height, STEPS):
            for x in range(0, self.width , STEPS):
                ...
                direction = direction / Alib.norm(direction)
    ...
    def cast_ray(self, orig, dir, origObj = None, recursion = 0):
        ...
        # Direccion de vista
        view_dir = Alib.sub(self.camPosition, intersect.point)
        view_dir = view_dir / Alib.norm(view_dir)
        ...
        if self.dirLight:
            ...
            intensity = max(0, Alib.punto(intersect.normal, light_dir)) * self.dirLight.intensity
            ...
            spec_intensity = self.dirLight.intensity * max(0, Alib.punto(view_dir, reflect)) ** material.spec
            ...
            finalSpecColor = Alib.sum(finalSpecColor, (1 - shadow_intensity) * specColor)
        ...
        for pointLight in self.pointLights:
            ...
            # Iluminacion difusa
            light_dir = Alib.sub(pointLight.position, intersect.point)
            light_dir = light_dir / Alib.norm(light_dir)
            intensity = max(0, Alib.punto(intersect.normal, light_dir)) * pointLight.intensity
            ...
            # Iluminacion especular
            reflect = reflectVector(intersect.normal, light_dir)
            spec_intensity = pointLight.intensity * max(0, Alib.punto(view_dir, reflect)) ** material.spec
            ...
            # Shadows
            shadInter = self.scene_intersect(intersect.point, light_dir, intersect.sceneObject)
            lightDistance = Alib.norm( Alib.sub(pointLight.position, intersect.point) ) # TENEMOS UNA ULTIMA IMPLEMENTACION PENDIENTE. 
            if shadInter and shadInter.distance < lightDistance:
                shadow_intensity = 1

            pLightColor = Alib.sum(pLightColor, (1 - shadow_intensity) * diffuseColor)
            finalSpecColor = Alib.sum(finalSpecColor, (1 - shadow_intensity) * specColor)
        ...
        if material.matType == OPAQUE:
            ...
            outside = Alib.punto(dir, intersect.normal) < 0
            bias = 0.001 * intersect.normal
            kr = fresnel(intersect.normal, dir, material.ior)

            reflect = reflectVector(intersect.normal, np.array(dir) * -1)
            reflectOrig = Alib.sum(intersect.point, bias) if outside else Alib.sub(intersect.point, bias)
            reflectColor = self.cast_ray(reflectOrig, reflect, None, recursion + 1)
            reflectColor = np.array(reflectColor)

            if kr < 1:
                refract = refractVector(intersect.normal, dir, material.ior )
                refractOrig = Alib.sub(intersect.point, bias) if outside else Alib.sum(intersect.point, bias)
