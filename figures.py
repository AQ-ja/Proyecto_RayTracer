...
class DirectionalLight(object):
    def __init__(self, direction = V3(0,-1,0), intensity = 1, color = WHITE ):
        self.direction = Alib.normalize(direction)
...
class Sphere(object):
    ...
    def ray_intersect(self, orig, dir):
        ...
        L = Alib.sub(self.center, orig)
        l = Alib.norm(L)

        tca = Alib.punto(L, dir)
        ...
        # P = O + t * D
        hit = Alib.sum(orig, t0 * np.array(dir) )
        normal = Alib.sub( hit, self.center )
        normal = normal / Alib.norm(normal) #la normalizo

        u = 1 - ((np.arctan2(normal[2], normal[0] ) / (2 * Alib.Pi())) + 0.5)
        v = np.arccos(-normal[1]) / Alib.Pi()
...
class Plane(object):
    def __init__(self, position, normal, material = Material()):
        self.position = position
        self.normal = normal / Alib.norm(normal)
        self.material = material

    def ray_intersect(self, orig, dir):
        #t = (( planePos - origRayo) dot planeNormal) / (dirRayo dot planeNormal)
        denom = Alib.punto(dir, self.normal)

        if abs(denom) > 0.0001:
            num = Alib.punto(Alib.sub(self.position, orig), self.normal)
            t = num / denom
            if t > 0:
                # P = O + t * D
                hit = Alib.sum(orig, t * np.array(dir))
...
class AABB(object):
    ...
    def __init__(self, position, size, material = Material()):
        ...
        #Sides
        self.planes.append(Plane( Alib.sum(position, V3(halfSizeX,0,0)), V3(1,0,0), material))
        self.planes.append(Plane( Alib.sum(position, V3(-halfSizeX,0,0)), V3(-1,0,0), material))

        # Up and down
        self.planes.append(Plane( Alib.sum(position, V3(0,halfSizeY,0)), V3(0,1,0), material))
        self.planes.append(Plane( Alib.sum(position, V3(0,-halfSizeY,0)), V3(0,-1,0), material))

        # Front and Back
        self.planes.append(Plane( Alib.sum(position, V3(0,0,halfSizeZ)), V3(0,0,1), material))
        self.planes.append(Plane( Alib.sum(position, V3(0,0,-halfSizeZ)), V3(0,0,-1), material))
